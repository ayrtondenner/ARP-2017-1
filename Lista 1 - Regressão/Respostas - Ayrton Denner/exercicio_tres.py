...
def calcula_coeficiente_de_correlacao(lista_x, lista_y):
    a = np.sum((lista_x - np.average(lista_x)) * (lista_y - np.average(lista_y)))
    b = np.sqrt(np.sum((lista_x - np.average(lista_x)) ** 2))
    c = np.sqrt(np.sum((lista_y - np.average(lista_y)) ** 2))
    return a / (b * c)
...
array_quartos = np.array([2, 2, 4, 3, 2, 3, 4, 2, 2, 3, 3, 2, 3, 2, 2, 3, 3, 2, 3, 3, 2, 2, 2, 3, 3, 2, 2, 3, 4, 2, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3])

array_garagem = np.array([2, 2, 2, 1, 1, 2, 1, 2, 2, 1, 1, 1, 1, 2, 1, 2, 2, 2, 1, 2, 2, 2, 2, 1, 2, 2, 1, 2, 2, 0, 2, 1, 2, 1, 2, 1, 2, 2, 2, 2])
...
matriz_x = np.matrix([array_vazio, array_tamanho, array_idade, array_andar, array_quartos, array_garagem]).T

matriz_y = np.matrix([array_preco]).T

coeficiente_correlacao_tamanho = calcula_coeficiente_de_correlacao(array_tamanho, array_preco)
coeficiente_correlacao_idade = calcula_coeficiente_de_correlacao(array_idade, array_preco)
coeficiente_correlacao_andar = calcula_coeficiente_de_correlacao(array_andar, array_preco)
coeficiente_correlacao_quartos = calcula_coeficiente_de_correlacao(array_quartos, array_preco)
coeficiente_correlacao_garagem = calcula_coeficiente_de_correlacao(array_garagem, array_preco)
